...
export async function up(db) {
  ...
  // To speed up listing selection for counters by cityId or categoryId
  await db.schema.createIndex('listing_cityId_key').on('listing').column('cityId').execute();
  await db.schema.createIndex('listing_categoryId_key').on('listing').column('categoryId').execute();

  // To speed up listing_subcategory selection for subcategory counters
  await db.schema
    .createIndex('listing_subcategory_subcategoryId_key')
...
export async function down(db) {
  ...
  await db.schema.dropIndex('listing_cityId_key').execute();
  await db.schema.dropIndex('listing_categoryId_key').execute();
  await db.schema.dropIndex('listing_subcategory_subcategoryId_key').execute();

  await db.schema.alterTable('category').dropColumn('activeListingAmount').execute();
  await db.schema.alterTable('subcategory').dropColumn('activeListingAmount').execute();
  await db.schema.alterTable('city').dropColumn('activeListingAmount').execute();
}
