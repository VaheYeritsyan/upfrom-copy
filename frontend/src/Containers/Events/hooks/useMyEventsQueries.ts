...
export const useMyEventsQueries = (
  isPause: boolean,
  ...
  const loadMore = () => {
    const pageInfo = data?.yourEvents.pageInfo;
  ...
  return {
    isLoadingMyEvents: fetching && !isLoadingMore && !isRefreshing,
    hasNextPage: data?.yourEvents.pageInfo.hasNextPage,
    loadMore,
    refresh,
    isLoadingMore,
    isRefreshing,
    myEvents: data?.yourEvents.edges.map(({ node }) => node) || [],
  };
};
